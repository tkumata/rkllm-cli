...
- [x] MCP ツールの取り扱い整理: 接続成功時に短縮版ツール一覧と必須引数優先の `<tool_call name="...">` サンプルを常時 system プロンプトに挿入し、ローカル書き込み優先を明文化。
...
- [x] 複数行入力のカーソル移動/挿入対応: グラフェム単位のバッファとカーソル位置を管理し、左右/上下矢印で移動できるようにする。再描画時にカーソル位置を計算して MoveTo する。文字入力・ペーストはカーソル位置に挿入し、Backspace/Delete もその位置基準に変更。全角/結合文字・改行/折返しを含むユニットテストを追加。
